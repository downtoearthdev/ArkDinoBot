...
public class BreedRequest implements Serializable {
    ...
    public void start() {
        new Timer().schedule(new RemoveNotifyMsgTimerTask(ArkDinoBot.getInstance().getFulfilChannel().getHistoryAround(msgId, 5).complete().getMessageById(msgId)), 4320000);
    }
